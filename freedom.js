...
function populate(){
    for(let i =0; i < manobj.length; i++){
        let current_pic = manobj[i].picture
        console.log(`manobj[i].song is ${manobj[i].song}`);
        if(manobj[i].picture){
            document.getElementById("songs").innerHTML+=`
            <div class="song" onclick="hello(${i})">
            <img class="pic" src="static/img/${current_pic}" alt="m">
            ...
            <p>${manobj[i].artist}</p>
            <div class="songsub" id="${i}">
            Click to Bring up Audio Controls
            </div>
            </div>
        `
        }
...
populate();
function hello(magic){
    console.log(`hello ${magic}`);
    document.getElementById(magic).innerHTML=`
    <audio controls src="static/audio/${manobj[magic].song}.mp3"></audio>
    `
}
